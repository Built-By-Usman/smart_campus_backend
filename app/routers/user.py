...
@router.get("/load-teacher-dashboard/", response_model=TeacherDashboardResponse)
def load_teacher_dashboard(
    db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)
):
    return teacher_dashboard(db=db, teacher_id=int(current_user.id))
...
def all_unauthenticated_teachers(
    db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)
):
...
def all_unauthenticated_students(
    db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)
):
...
def approve_unauthenticated_user(
    data: ApproveRejectUserSchema,
...
def decline_unauthenticated_user(
    data: ApproveRejectUserSchema,
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(get_current_user),
):
