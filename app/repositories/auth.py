...
def login_user(request:OAuth2PasswordRequestForm ,db:Session):
    ...
    if not admin:
        ...
        if not user.is_active:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail="You are not allowed to access the system")
        if not user.is_authenticated:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail="Please ask admin to approve your account")
